...
class EmbeddingCache(object):
    ...
    @staticmethod
    @retry(exceptions=Exception, tries=3, max_delay=20)
    def get_bce_embedding(req_text: str):
    ...
    def build_with_context_base(self, context_type: str):
        with open("dataset/val_dataset.json", "r", encoding="utf-8") as f:
        ...
        for i in tqdm(range(query_num), desc="generate embedding"):
            embedding_data[i] = self.get_bce_embedding(queries[i])
        np.save(f"dataset/{context_type}_m3e_embedding_base.npy", embedding_data)

    def build_base(self):
        self.build_with_context_base("queries") 
        self.build_with_context_base("corpus") 

    @staticmethod
    def load(query_write=False): # load all embeddings
        current_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        
        queries_embedding_data = np.load(os.path.join(current_dir, "dataset/queries_m3e_embedding_base.npy"))
        corpus_embedding_data = np.load(os.path.join(current_dir, "dataset/corpus_m3e_embedding_base.npy"))
        ...
        if query_write:
            rewrite_queries_embedding_data = np.load(os.path.join(current_dir, "dataset/query_rewrite_openai_embedding.npy"))
            with open("dataset/query_rewrite.json", "r", encoding="utf-8") as f:
